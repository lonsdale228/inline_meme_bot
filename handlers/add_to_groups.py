...
from database.utils import (
    get_user_groups,
    send_meme_to_selected_group,
    add_meme_to_all_user_groups,
)
...
async def add_meme_to_group(message: Message, state: FSMContext, meme_id: int):
    ...
    await message.answer(
        f"Choose groups to add meme to: \nOr ignore, to keep it private",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard),
    )


@router.callback_query(
    F.data.contains("change"), StateFilter(AddToGroup.add_meme_to_group)
)
async def callback_change(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        text=f"Choose groups to add meme to:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard),
    )


@router.callback_query(
    F.data.contains("send"), StateFilter(AddToGroup.add_meme_to_group)
)
async def callback_send(callback: CallbackQuery, state: FSMContext):
    ...
    if "selected" in callback.data:
        ...
        await add_meme_to_all_user_groups(
            user_id=str(callback.from_user.id), meme_id=meme_id
        )

    await callback.message.edit_text(
        "Successfully sent meme!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[])
    )
    await state.clear()


@router.callback_query(
    F.data.contains("left_private"), StateFilter(AddToGroup.add_meme_to_group)
)
